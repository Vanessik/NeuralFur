...
class GaussianModel:
    ...
    def load_ply_mesh(self, path, spatial_lr_scale):

        self.spatial_lr_scale = spatial_lr_scale

        
        #because smplx tracking produce very big head
        points = np.array(trimesh.load(path).vertices)
        colors = np.ones_like(points)

        
        fused_point_cloud = torch.tensor(np.asarray(points)).float().cuda()
        fused_color = RGB2SH(torch.tensor(np.asarray(colors)).float().cuda())
        features = torch.ones((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0
        ...
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._xyz_frame_init = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))
        self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = nn.Parameter(rots.requires_grad_(True))
