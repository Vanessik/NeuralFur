...
def readSyntheticSceneInfo(path, images, eval, llffhold=5, scale_factor=1, resolution=(1024, 1024), num_views=-1):
    try:
    ...
    for world_mat in camera_dict:
        ...
        intrinsics_all.append(scale_matrix(torch.from_numpy(intrinsics).float(), scale_factor))
    ...
    reading_dir = "images" if images == None else images
    cam_infos_unsorted = readSyntheticCameras(extrinsics_all=pose_all_inv, intrinsics_all=intrinsics_all, images_folder=os.path.join(path, reading_dir), resolution=resolution)
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)

    if eval:
        if num_views > -1:
            if num_views == 16:
                idxes_list = [8, 27, 16, 35, 4, 12, 23, 31, 2, 14, 20, 25, 29, 32, 6, 10]
            elif num_views == 8:
                idxes_list = [8, 27, 16, 35, 4, 12, 23, 31]
            elif num_views == 4:
                idxes_list = [8, 27, 16, 35]
            elif num_views == 2:
                idxes_list = [8, 27]
            elif num_views == 1:
                idxes_list = [0]

            train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx in idxes_list]
            test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx not in idxes_list]
        else:
            train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
            test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
    else:
    ...
    print('len cams', len(train_cam_infos), len(test_cam_infos))
