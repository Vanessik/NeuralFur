...
class GaussianModelHair:
    ...
    def __init__(self, data_dir, strands_config, sh_degree):
        ...
        self._strand_gravity = torch.empty(0)
        ...
        self.strands_generator = OptimizableTexturedStrands(
            **strands_config['textured_strands'], num_guiding_strands=num_guiding_strands
        ).cuda()
    ...
    def restore(self, model_args, training_args, training_args_hair):
        (
            self._scaling,
            self.active_sh_degree, 
            gen_dict,
        ...
        self.training_setup(training_args, training_args_hair)
        self.optimizer.load_state_dict(opt_dict)
        self.scheduler.load_state_dict(shd_dict)
        
            
        if self.strands_generator.strand_length_scale is not None:
            self.strands_generator.init_length_scale()
            self.strands_generator.init_strand_groups()

        self.strands_generator.init_pos_enc()
    ...
    def generate_strands(self, iter, num_strands = -1):
        if num_strands != -1:
            p, _, _, _, _, z, self._strand_gravity = self.strands_generator.forward_inference(num_strands)
            diffusion_dict = {}
        else:
            p, _, _, _, _, z, diffusion_dict, self._strand_gravity = self.strands_generator(iter)
    ...
    def create_from_pcd(self, data_path, scale = 5e-1):
        ...
        try:
            ...
            self.scale = scale * torch.ones(1, device="cuda")
