...
def render_hair(viewpoint_camera, pc : GaussianModel, pc_hair: GaussianModelHair, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, body_scale=0.0):
    """
    Render the scene. 
    ...
    """
    if body_scale < 0:
        original_scaling = pc._scaling.data.clone()

        pc._scaling.data.fill_(body_scale)    
